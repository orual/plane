...
class CycleListCreateAPIEndpoint(BaseAPIView):
    ...
    def post(self, request, slug, project_id):
        ...
        if (request.data.get("start_date", None) is None and request.data.get("end_date", None) is None) or (
            ...
            serializer = CycleCreateSerializer(data=request.data, context={"request": request, "project_id": project_id})
...
class CycleDetailAPIEndpoint(BaseAPIView):
    ...
    def patch(self, request, slug, project_id, pk):
        ...
        serializer = CycleUpdateSerializer(cycle, data=request.data, partial=True, context={"request": request, "project_id": project_id})
        if serializer.is_valid():
            if (
                request.data.get("external_id")
